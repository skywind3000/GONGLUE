...
你刚刚结束了在当地战士公会的学生生涯。经过大量的练习和汗水，你终于完成了训练，准备开始一场充满危险的冒险。作为对你技能的测试，当地的公会大师们派你进入厄运地牢。你的任务是带回Yendor护身符。完成这个任务的奖励将是在当地公会的全额会员资格。此外，你还可以保留你从地牢带回的所有战利品。
...
你刚刚开始了一场游戏《Rogue》。你的目标是尽可能多地收集宝藏，找到耶多尔护身符，并活着离开末日地牢。屏幕上显示了你在当前地牢层级所经过的地方和所见到的事物的地图。随着你探索更多的层级，这些信息会显示在你面前的屏幕上。

在《Rogue》游戏中，所有命令都是一到两个按键，命令的结果会在屏幕上以图形方式显示。游戏至少需要 24 行 80 列的屏幕大小。如果屏幕更大，也只会使用 24x80 的区域来显示地图。

## 3. 屏幕上的那些东西都是什么意思？

为了理解《流氓》游戏中的情况，你首先需要了解游戏是如何使用屏幕的。图1 是一个《流氓》游戏屏幕可能的样子的示例。
...
| `%` | 通往其他层级的楼梯。|
| `:` | 一份食物。|
| `A-Z` | 大写字母代表《末日地牢》中的各种居民。小心，他们可能会很凶恶和危险。|
...
## 8. 选项
由于个人品味和对流氓行为方式的不同理解，您可以设置一组选项，使流氓以各种不同的方式表现。

### 设置选项

有两种方法可以设置选项。第一种是使用流氓的 'o' 命令；第二种是使用  `ROGUEOPTS` 环境变量。

##### 使用 'o' 命令

当您在流氓中输入 'o' 时，它会清除屏幕并显示所有选项的当前设置。然后将光标放在第一个选项的值旁边，等待您输入。您可以输入 `<ENTER>` 以转到下一个选项，输入 '-' 以返回上一个选项，输入 `<ESC>` 以返回游戏，或者为选项赋值。对于布尔选项，只需输入 't' 表示真或 'f' 表示假。对于字符串选项，输入新值并按  `<ENTER>`。
...
```
C:>set ROGUEOPTS=jump,nopassgo,name:Blue Meanie
```
...
- `passgo` (`passgo`)：在通道中跟随转弯。如果你在通道中奔跑并撞上石头或墙壁，rogue 会查看是否可以向右或向左转。如果只能向一个方向转，它会转向那个方向。如果可以向两个方向转或都不能转，它会停下来。这个算法有时会导致一些稍微令人困惑的情况，这就是为什么默认设置为不随走随停。
- `skull` (`skull`)：如果你被杀死，打印出一个骷髅或墓碑。墓碑选项也会做同样的事情。
- `color` (`color`)：绘制地图时使用漂亮的颜色。
- `PCgraphics` (`PCgraphics`)：使用扩展的 ASCII 字符，就像 IBM PC rogue 一样。
